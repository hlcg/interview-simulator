...
async def log_requests(request: Request, call_next):
    ...
    logger.info(f"📍 [MIDDLEWARE_IN] ➡️  {method} {path}")
    
    try:
        # Appelle la route suivante
        response = await call_next(request)
        logger.info(f"📍 [MIDDLEWARE_OUT] ✅ {method} {path} → Status {response.status_code}")
        return response
        
    except Exception as e:
        logger.error(f"❌ [MIDDLEWARE_ERROR] {method} {path} → Exception: {type(e).__name__}: {e}")
        logger.error(f"❌ [MIDDLEWARE_ERROR] Traceback:\n{traceback.format_exc()}")
        raise

logger.info("🔧 [4/10] HTTP Logging middleware added")

# Event handler pour initialiser la DB au démarrage
@app.on_event("startup")
def startup_event():
    logger.info("🔧 [5/10] STARTUP EVENT TRIGGERED")
    try:
        logger.info("🔧 [5.1/10] Calling init_db()...")
        init_db()
        logger.info("✅ [5.2/10] Database initialized successfully")
    except Exception as e:
        logger.error(f"❌ [5.3/10] ERROR initializing database: {e}", exc_info=True)

logger.info("🔧 [6/10] Startup event handler registered")

# Lazy loading du client Anthropic
_client = None
logger.info("🔧 [7/10] Client variable initialized")
...
logger.info("🔧 [8/10] get_client function defined")

# Base de données SQLite
DB_PATH = "interview_sessions.db"
logger.info(f"🔧 [9/10] DB_PATH set to: {DB_PATH}")
...
logger.info("🔧 [10/10] init_db function defined")
...
logger.info("🔧 [11/10] Pydantic models defined")

# ==================== ROUTES ====================

@app.get("/api/health")
def health():
    logger.info("📍 [ROUTE] /api/health - ROUTE HANDLER CALLED")
    try:
        result = {"status": "ok", "service": "interview-backend"}
        logger.info(f"✅ [ROUTE] /api/health returning: {result}")
        return result
    except Exception as e:
        logger.error(f"❌ [ROUTE] /api/health ERROR: {e}", exc_info=True)
        return {"error": str(e)}

@app.post("/api/generate-questions")
def generate_questions(req: GenerateQuestionsRequest):
    """Génère des questions d'entretien basées sur le secteur, l'offre et le CV"""
    logger.info(f"📍 [ROUTE] /api/generate-questions called with sector={req.sector}")
    try:
...
def analyze_answer(req: AnalyzeAnswerRequest):
    ...
    logger.info(f"📍 [ROUTE] /api/analyze-answer called")
    try:
        client = get_client()
...
def generate_report(req: GenerateReportRequest):
    ...
    logger.info(f"📍 [ROUTE] /api/generate-report called")
    try:
        client = get_client()
...
def start_session(req: StartSessionRequest):
    ...
    logger.info(f"📍 [ROUTE] /api/session/start called")
    try:
        import uuid
...
def save_answer(req: AnswerRequest):
    ...
    logger.info(f"📍 [ROUTE] /api/session/answer called for session {req.session_id}")
    try:
        client = get_client()
...
def complete_session(req: CompleteSessionRequest):
    ...
    logger.info(f"📍 [ROUTE] /api/session/complete called for session {req.session_id}")
    try:
        client = get_client()
...
def get_history():
    ...
    logger.info(f"📍 [ROUTE] /api/session/history called")
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
...
def get_session(session_id: str):
    ...
    logger.info(f"📍 [ROUTE] /api/session/{session_id} called")
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
...
def get_evolution():
    ...
    logger.info(f"📍 [ROUTE] /api/session/stats/evolution called")
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
...
def text_to_speech(request: dict):
    ...
    logger.info(f"📍 [ROUTE] /api/tts called")
    try:
